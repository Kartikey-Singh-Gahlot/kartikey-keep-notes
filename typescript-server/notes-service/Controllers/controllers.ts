...
export async function notesService(request:Request, response:Response):Promise<Response>{
  const responsePayLoad:ResponseEntity<string>={
   ...
   try{
       ...
       responsePayLoad.body="Notes Service Reachable :)";
       return response.status(200).json(responsePayLoad);

   }
   catch(err){
      responsePayLoad.code="INTERNAL_SERVER_ERROR",
      responsePayLoad.body="Notes Service Not Reachable !"
