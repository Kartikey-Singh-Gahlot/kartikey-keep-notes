...
export function SigninOtpVerificationBox({password, email}){
    ...
    function trgrChange(e){
      ...
      const otp = Object.values(updatedForm).join("");
      if(otp.length===4){
        trgrFormSubmit(e);
        return;
      }
    ...
    return(
        <form className="flex flex-col items-center p-3 rounded-2xl gap-5" onSubmit={(e)=>{trgrFormSubmit(e)}}>
             ...
             <p className="w-full text-[12px] text-justify">An email has been sent to the registered email id, enter here for verification</p>
...
export  function SignupOtpVerificationBox(){
    ...
    async function trgrFormSubmit(e){
      ...
      try{
        ...
        toast.error(pr.body, {duration:2000});
        setShake(true);
        setTimeout(()=>{
    ...
    return(
        <form className="flex flex-col items-center p-3 rounded-2xl gap-5" onSubmit={(e)=>{trgrFormSubmit(e)}}>
             ...
             <div className="flex"><h1 className="px-1">didn't received mail ?</h1><button className="underline">resend</button></div>
        </form>
    )
}