...
import { SectionalLoader } from "../components/loader.js";
...
export default function ContactForm({lightTheme}){
    ...
    return(
       ...
       (<form  onSubmit={(e)=>{trgrFormSubmit(e)}} className="flex flex-col  gap-2 p-4 rounded-2xl ">
            <input  required  onChange={(e)=>{ trgrFormChange(e)}} value={formData.sender} name="sender" className="rounded-[5px] border-green-800 border-[1.5px] w-[30vw] min-w-[250px] h-10 text-center placeholder-gray-400 text-[14px]" placeholder="xyz@gmail.com"/>
            <textarea onChange={(e)=>{ trgrFormChange(e)}} value={formData.message}    name="message"    className="rounded-[5px] border-green-800 border-[1.5px]  p-1.5 min-h-40  placeholder:text-gray-400 text-[14px]"                    placeholder="Your message for us"/>
